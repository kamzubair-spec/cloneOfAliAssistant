...
public sealed class StoryAnalyzerService
{
    ...
    {
        if (storyContent.HasReadableInlineImages && AiProviderSettings.UseOpenAiForInlineImages)
        {
            var systemPrompt = BuildSystemPrompt(repoPath);
            var (userContent, imageCount) = BuildVisionContent(storyContent);
            if (imageCount == 0)
            {
...
""";
        ...
        var (baseContent, imageCount) = BuildVisionContent(storyContent);
        if (imageCount == 0)
        {
            return "Inline image blocks were found, but none resolved to readable local image files for OpenAI vision.";
        }

        var userContent = baseContent
            .Concat(new[]
            {
    ...
    private static (IReadOnlyList<AiChatContentPart> Content, int ImageCount) BuildVisionContent(JiraStoryAnalysisContent storyContent)
    {
        var content = new List<AiChatContentPart>();
        var imageCount = 0;
        foreach (var block in storyContent.Blocks)
        {
            if (block.Kind.Equals("image", StringComparison.OrdinalIgnoreCase) && block.IsInlineImage)
            {
